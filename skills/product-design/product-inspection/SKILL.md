...
4. **Get the product running first — every run, every mode.** A product inspection is worthless if it's about the product as *described* or as *coded* instead of as *shipped*. At check-in: ask for a production/staging URL or local launch instructions, **then** check your own available tools for a browser bridge and make one real call to actually load and confirm you can see the live product. Record the outcome in `product-inspection/GARAGE.md`'s access map. Six of the ten stations (2 best practices, 3 accessibility, 5 usability, 6 visual design, 7 performance, 8 security & privacy) can only produce `[verified]` findings against a running instance — skipping this probe silently downgrades most of the inspection to `[reported]`, and the user finds out at the end.
   ...
   **Probe the design source the same way.** Intake asks whether there's a Figma file or spec the product was built from. Asking is not reaching: check your own available tools for anything Figma-shaped, make one real call against their actual file, and record the result in the access map alongside the running-instance row. This is what turns Station 6's "compare against the stated ideal" from a question you ask the user into a comparison you actually perform, and it's what lets Station 1 see design-side adoption drift that never shows up in the DOM. `reference/FIGMA-ACCESS.md` covers what each bridge can see and what the four access states cost you. It's important to know that a product with no design file is a perfectly normal answer — record it and move on. What's not fine is collecting the Figma link at check-in and never opening it.
