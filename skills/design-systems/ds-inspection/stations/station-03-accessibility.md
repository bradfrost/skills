...
2. **Compute contrast from the tokens, and say which tokens.** Body text 4.5:1, large text and non-text 3:1 — across *every theme/mode*, not just the default. Dark mode is where contrast quietly dies. **Name your source in the finding:** design variables and code tokens can disagree, and when they do, one side is shipping a failure while the other passes. If you can reach both, compute from both and report the delta; if you can only reach one, say which.
