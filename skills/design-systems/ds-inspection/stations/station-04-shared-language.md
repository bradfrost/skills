...
2. **Token naming algorithm:** is there one coherent scheme, or competing patterns (`typography-body-large` vs `large-body-text`)? Are tokens named by **role** (`color.action.primary`) or raw value (`blue-500`) at the semantic tier? Old and new schemes coexisting with no migration? This station judges the scheme *within* each side; **Station 6 step 4 diffs the two lists against each other**, which is where delimiter and word-break drift (`spacing/m` vs `spacing-m`, `demibold` vs `demi-bold`) actually surfaces. If you're running this station standalone, do that diff here.
